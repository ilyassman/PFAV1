...
async function updatedialog(id) {
    ...
    butupdate.onclick = function (e) {
        ...
        if (emailu.value != data[0].email) {
            isEmailexists(emailu.value).then((isexists) => {
                if (isexists) {
                    document.getElementById("erreuremailu").innerHTML =
                        "Email deja existe";
                    document.getElementById("erreuremailu").style.display =
                        "block";
...
name.onkeyup = () => {
    champvide(
        ...
        document.getElementById("erreurenom")
    );
};
prenom.onkeyup = () => {
...
butfor.onclick = function (e) {
    e.preventDefault();
    champvide(name,  document.getElementById("erreurenom"));
    champvide(prenom, document.getElementById("erreureprenom"));
    champemailvalid(email, document.getElementById("erreuremail"));
    champphonelvalid(tele, document.getElementById("erreurphone"));
    champpassvalid(password, document.getElementById("erreurpass"));
    ...
    if (
        ...
        isEmailexists(email.value).then((isexists) => {
            if (isexists) {
                ...
                Swal.fire({
                    ...
                    didOpen: () => {
                        Swal.showLoading();
                        adduser(email.value, password.value, tele.value, 1)
                            .then((id) => {
                                addformateur(
                                    name.value,
                                    prenom.value,
...
async function updateForm(
    ...
    const urlAPI = `http://127.0.0.1:8000/api/v1/formateurs/${id}`;
    console.log(
        JSON.stringify({ email, password, num_tel, nom, prenom, description })
    );
